...
const upload = multer({
	...
	limits : { fileSize : 5 * 1024 * 1024 }, // 업로드할 파일 용량, 최대 업로드 용량 5mb
});
...
app.use(session({
	resave : false, // 같은 값을 저장할 때 다시 저장할 것인가? false - 저장 안하고 기존값 유지
...
/**
	...
	upload.fields([
		...
		....
		{ name : 'name 속성값' },
	]) - req.files : 없로드된 파일 데이터
*/

app.post("/upload", upload.single('image'), (req,res) => {
	console.log(req.file); // 한개인경우
	return res.send("");
});

app.post("/upload2", upload.array("images"), (req, res) => {
	console.log(req.files); // 파일 여러개인 경우
	return res.send("");
});
...
app.use((req,res,next) => {
	const err = new Error(`${req.url}는 없는 페이지입니다`);
	err.status = 404;
	next(err);
});
